...
Officer: 6219282
CaseNum: 801-3-95643797-6219282

One last chance to record the Cooley gang before the curtain comes down on the Marriage of Figaro. They might be the most influential group in console city but they still have their quirks and will be spotable thanks their trademark scarf.
...
You’ll want to set the recognised property of the theatregoers array to true when the equivalent element of the gangMember is also true.
...
function draw()
{
	...
	{
		for (let j = 0; j < theatregoers[i].length; j++)
		{
			if (gangMember[i*10 + j])
			{
				theatregoers[i][j].recognised = true;
			}
		}
	}
...
function highlight()
{
	...
	for (var i = 0; i < theatregoers.length; i++)
	{
		...
		{
			if (theatregoers[i][j].recognised)
			{
